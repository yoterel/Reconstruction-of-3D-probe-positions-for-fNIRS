function affineEstData = nfri_mni_estimation_spm(Origin, Others)
% NFRI_MNI_ESTIMATION - Execute probablistic registration.
%
% USAGE.
%   nfri_mni_estimation;
...
%
% OUTPUTS.
%   Results are saved in the same directory with this script as Excel
%   format.
%
% EXAMPLE.
...
%
% Ippeita Dan
% Centre for Development of Advanced Medical Technology
% Jichi Medical University 
% -------------------------------------------------------------------

fullPathList.Origin = Origin;
if ischar(Others) % Others: file name of optode positions
    fullPathList.Others = Others;
    OthersXYZ = ReadCSV(fullPathList.Others, 0);
else % Others: matrix of optode positions
    OthersXYZ = Others;
end
...
% OthersXYZ = [OthersXYZ; xyz_ch];

% --------------------------------------------------------------
% Result will be stored on the path where origin file is placed.
% We assign timestamp as the filename.
% --------------------------------------------------------------

fullPathList.Rep = fileparts(fullPathList.Origin);
T = clock;
FileNameRep = sprintf('%04d%02d%02d-%02d%02d%02d.xls', ...
    T(1), T(2), T(3), T(4), T(5), round(T(6)));
fullPathList.Rep = fullfile(fullPathList.Rep, FileNameRep);

% uiSetup(fullPathList, OriginXYZ, OthersXYZ);

ThisScriptPath = GetMatDir;
Selected = find(sum(isnan(OriginXYZ),2) == 0);
...
loadedData.SelIndx = Selected'; % Pick up the index from 10-20 points.
...
function [affineEstData, hFigure] = AffineEstimation4(loadedData)
...
% stored in ?eOrigin'...is affine-transformed to the ideal 10-20 orientation
% on the MNI space
%----------------------------------------------------------
...
%----------------------------------------------------------
% 10-20 standard points of a subject in the real world coordinate system
% stored in ?eOrigin'...is affine-transformed to the 10-20 orientation
% on reference brains
%----------------------------------------------------------
...
%==========================================================
% Then, transforming given head surface points stored in 'Others?f
...
OtherC      = ones(size(OtherH));   % !!!!! % ˆÓ–¡•s–¾‚ÈƒRƒ?ƒ“ƒg
% OtherCMean  = ones(PointN,3);       % OtherCMean ‚ÍŽQ?Æ‚³‚ê‚Ü‚¹‚ñ?B
...
xlswrite(fullPathRep, affineEstData.RefN, 'Info', 'A2');  % ‚æ‚­Ž¸”s‚·‚é
...
function XYZ = ReadCSV(File, LabelRowNum)
XYZ = [];
